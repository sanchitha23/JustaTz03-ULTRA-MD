...
async function filesInit(pluginFolder = pluginFolder, pluginFilter = pluginFilter, conn) {
  ...
  await Promise.all(
    readdirSync(folder)
      ...
      .map(async filename => {
        try {
          let file = 𝗙R𝗘𝗘 𝗜𝗡𝗧𝗘R𝗡𝗘𝗧🇹🇿.__filename(join(folder, filename))
...
async function reload(
  ...
  if (pluginFilter(filename)) {
    let dir = 𝗙R𝗘𝗘 𝗜𝗡𝗧𝗘R𝗡𝗘𝗧🇹🇿.__filename(join(pluginFolder, filename), true)
    ...
    if (err) conn.logger.error(`syntax error while loading '${filename}'\n${format(err)}`)
      ...
      try {
        const module = await importFile(𝗙R𝗘𝗘 𝗜𝗡𝗧𝗘R𝗡𝗘𝗧🇹🇿.__filename(dir)).catch(console.error)
