...
J𝖀𝗦𝐓Λ-𝗧𝙕.__filename = function filename(pathURL = import.meta.url, rmPrefix = platform !== 'win32') { return rmPrefix ? /file:\/\/\//.test(pathURL) ? fileURLToPath(pathURL) : pathURL : pathToFileURL(pathURL).toString() }; J𝖀𝗦𝐓Λ-𝗧𝙕.__dirname = function dirname(pathURL) { return path.dirname(J𝖀𝗦𝐓Λ-𝗧𝙕.__filename(pathURL, true)) }; J𝖀𝗦𝐓Λ-𝗧𝙕.__require = function require(dir = import.meta.url) { return createRequire(dir) } 

J𝖀𝗦𝐓Λ-𝗧𝙕.API = (name, path = '/', query = {}, apikeyqueryname) => (name in J𝖀𝗦𝐓Λ-𝗧𝙕.APIs ? J𝖀𝗦𝐓Λ-𝗧𝙕.APIs[name] : name) + path + (query || apikeyqueryname ? '?' + new URLSearchParams(Object.entries({ ...query, ...(apikeyqueryname ? { [apikeyqueryname]: global.APIKeys[name in global.APIs ? global.APIs[name] : name] } : {}) })) : '')
